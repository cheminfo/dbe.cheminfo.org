...
# SERVER_ROOT is a tmpfs, so this is the one writable place in the container and
...
# Where this deployment answers: `/` on a host of its own, `/dbe/` as one tool
# among several on a shared one. The build carries no mount, so this is the only
# thing that differs between the two — one image, one tag, two addresses, no
# rebuild.
base_path="${BASE_PATH:-/}"
case "$base_path" in
  /*) ;;
