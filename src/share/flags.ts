 ...
 *
 * Without it a shared calculator would open with whatever layers the receiver's
 * own browser has stored, so the figure a teacher composed — the working under
 * the sulfone, say — would not be the figure the class sees.
 */

import type { DisplayFlagKey } from '../state/displayFlags.ts';
import { DISPLAY_FLAGS, isDisplayFlagKey } from '../state/displayFlags.ts';

/** Query parameter listing the layers that are on: `?flags=breakdown`. */
...
/**
 * Write a `flags` parameter, in the order the chips show the layers.
 ...
 * a plain visit to the tool must not grow a query string.
