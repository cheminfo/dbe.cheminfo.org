 ...
 * is: the empty string on a host of its own, `/dbe` as one tool among several
 * on a shared one. It is read off the page rather than off the build — vite
 * writes the assets relative, so the build carries no mount at all, and what
 * tells two deployments apart is the `<base>` the container stamps in when it
 * starts. One image therefore serves both addresses.
 *
